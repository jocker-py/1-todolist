...
const Todolist: FC<TodolistType> = (todolist) => {
  ...
  const tasks = useSelector<AppRootStoreType, Array<TaskType>>(
  ...
  const onChangeTodolistTitle = useCallback(
    (title: string) => dispatch(changeTodolistTitle(title, id)),
    [dispatch, id]
  );
  ...
  const addTaskItem = useCallback(
    (title: string) => dispatch(addTask(title, id)),
    [dispatch, id]
  );
